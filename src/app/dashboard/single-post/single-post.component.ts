...
export class SinglePostComponent implements OnInit {
  ...
  addComment() {
    this.postService.addComment(this.post.postId, this.comment).subscribe((data)=>{
      ...
      this.fullComment.push(new Comment(commentId, postId, userId, content, timestamp));
    });
    this.comment = "";
  }
    ...
    if(!this.commentsLoaded) {
      this.postService.getComments(this.post.postId).subscribe((data:Array<any>)=> {
        for(let commentData of data) {
          this.fullComment.push(new Comment(commentData.commentId, commentData.postId, commentData.userId, commentData.contentString, commentData.timestamp));
        }
